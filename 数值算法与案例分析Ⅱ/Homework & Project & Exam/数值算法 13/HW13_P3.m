...
x = linspace(-1, 1, n+2); % x grid points, including boundary points
y = linspace(-1, 1, n+2); % y grid points, including boundary points
U(1, :) = x + 1;    % Set u(x, -1) = x + 1
U(n+2, :) = x + 1;  % Set u(x, 1) = x + 1
U(:, 1) = (y.^2 - 1)';    % Set u(-1, y) = y^2 - 1
U(:, n+2) = (y.^2 + 1)';  % Set u(1, y) = y^2 + 1
...
figure;
[X, Y] = meshgrid(-1:h:1, -1:h:1); % Create meshgrid for plotting
